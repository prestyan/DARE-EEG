...
class SleepConvHead(nn.Module):
    ...
    def __init__(self,
        ...
        # adapt time space (sampling)
        f_orig, f_targ = orig_targ_samp
        k = int(round(1.0 * f_orig / f_targ))
        if k % 2 == 0:
            k += 1
        kernel_size = max(k, 3)
        '''
        layers += [Conv1dWithConstraint(in_channels, in_channels,
                          kernel_size=kernel_size,
                          padding=kernel_size // 2,
                          bias=False, groups=in_channels, max_norm=1),
                   nn.BatchNorm1d(out_channels),
                   nn.GELU()]
        '''
        layers = []
    ...
    def forward(self, x):  # x: (B, C, T)
        if self.use_sampling:
            x = temporal_interpolation(x, self.out_time_length)
        x = self.conv_layers(x)  # -> (B, C1, T')
        return x
        '''
        if Tprime == T1:
