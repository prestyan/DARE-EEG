...
@torch.no_grad()
def get_metrics(output, target, metrics, is_binary=False, threshold=0.5):
    if is_binary:
        ...
        need_macro = 'precision_macro' in metrics
        need_weighted = 'precision_weighted' in metrics

        base_metrics = [m for m in metrics if m not in ('precision_macro', 'precision_weighted')]

        results = multiclass_metrics_fn(
            target, output, metrics=base_metrics
        )

        if need_macro or need_weighted:
            y_pred = np.argmax(output, axis=-1)

            # 计算并补充到 results
            if need_macro:
                results['precision_macro'] = precision_score(
                    target, y_pred, average='macro', zero_division=0
                )
            if need_weighted:
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, optimizer_disc=None,
    ...
    epoch_name = args.model_save_name + str(epoch)
