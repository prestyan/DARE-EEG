...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x).transpose(1, 3)  # B, N, C, D
        return x
...
class EEGTransformerReconstructor(nn.Module):
    ...
    def forward(self, x, chan_ids=None, mask_x=None, mask_y=None):
        ...
        # -- map from predictor-dim to reconstructor-dim
        ...
        if mask_y is not None:
            ...
            # mask_token = nn.Parameter(torch.zeros(1, 1, reconstructor_embed_dim))
...
class EEGTransformerPredictor(nn.Module):
    ...
    def forward(self, x, mask_x=None, mask_t=None): # mask_x yes! mask_t no!
        # conditions: (Nq, D) as qurey for downstream 
        ...
        if mask_x is not None:
            mask_x = mask_x
            # The original mask_x has shape [mN, mC], where each value is t * C + c. 
            # Dividing by the number of channels C and taking the integer part extracts the time position t.
            ...
            # -- concat query mask_token ys
            N_y = mask_t.shape[0]
            # mask_token = nn.Parameter(torch.ze ros(1, 1, embed_num, predictor_embed_dim))
        ...
        x = self.predictor_proj(x)
         ...
         # Output the complete time-series representation (original visible portion + predicted portion that was previously obscured).
...
class EEGTransformer(nn.Module):
    ...
    def forward(self, x, chan_ids=None, mask_x=None, mask_t=None):
        ...
        # -- add channels positional embedding to x
        x = x + self.chan_embed(chan_ids.long()).unsqueeze(0)  # (1,C,D) -> (1,1,C,D)
        ...
        # -- concat summary token self.summary_token = nn.Parameter(torch.zeros(1, embed_num, embed_dim))
        ...
        x = x[:, -summary_token.shape[1]:, :]  # BmN, embed_num, D

        if self.norm is not None:
            x = self.norm(x)

        x = x.flatten(-2)  # BmN, embed_num * D
